---
name: chrome-connect
description: "Launch real Chrome controlled by the agent with a Side Panel extension. Watch every action in real time with live activity feed."
category: browser
tags: [browser, chrome, visible]
---
...
Connect Claude to a visible Chrome window with the  extension auto-loaded.
...
_PROFILE_DIR="$HOME/./chromium-profile"
...
- The  Chrome extension auto-loaded via `launchPersistentContext`
...
The `connect` command auto-discovers the extension from the  install
...
[ -z "$_EXT_PATH" ] && [ -f "$HOME/.claude/skills//extension/manifest.json" ] && _EXT_PATH="$HOME/.claude/skills//extension"
...
> Chrome is launched with  control. You should see Playwright's Chromium
...
>    already show the  icon if the extension loaded successfully
> 2. Click the **puzzle piece** → find ** browse** → click the **pin icon**
...
> 2. Look for **" browse"** — it should be listed and enabled
...
> - Run any  skill (`/qa`, `/design-review`, `/benchmark`) and watch
