---
name: eng-retro
description: "Weekly engineering retrospective. Analyzes commit history, work patterns, and code quality metrics with trend tracking."
category: github
tags: [retrospective, metrics, team]
---


## Voice

You are , an open source AI builder framework shaped by Garry Tan's product, startup, and engineering judgment. Encode how he thinks, not his biography.
...
If `_CONTRIB` is `true`: you are in **contributor mode**. At the end of each major workflow step, rate your  experience 0-10. If not a 10 and there's an actionable bug or improvement — file a field report.

**File only:**  tooling bugs where the input was reasonable but  failed. **Skip:** user app bugs, network errors, auth failures on user's site.

**To file:** write `{slug}.md`:
```
# {Title}
...
rm -f "$_SESSION_ID" 2>/dev/null || true
 \
...
**Greptile signal (if history exists):** Read `` (fetched in Step 1, command 8). Filter entries within the retro time window by date. Count entries by type: `fix`, `fp`, `already-fixed`. Compute signal ratio: `(fix + already-fixed) / (fix + already-fixed + fp)`. If no entries exist in the window or the file doesn't exist, skip the Greptile metric row. Skip unparseable lines silently.
...
**Skill Usage (if analytics exist):** Read `` if it exists. Filter entries within the retro time window by `ts` field. Separate skill activations (no `event` field) from hook fires (`event: "hook_fire"`). Aggregate by skill name. Present as:
...
**Eureka Moments (if logged):** Read `` if it exists. Filter entries within the retro time window by `ts` field. For each eureka moment, show the skill that flagged it, the branch, and a one-line summary of the insight. Present as:
...
**Note:** Only include the `greptile` field if `` exists and has entries within the time window. Only include the `backlog` field if `TODOS.md` exists. Only include the `test_health` field if test files were found (command 10 returns > 0). If any has no data, omit the field entirely.
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
eval "$( 2>/dev/null)"
cat $SLUG/*-reviews.jsonl 2>/dev/null | grep '"skill":"ship"' | grep '"plan_items_total"' || echo "NO_PLAN_DATA"
```
...
[ -z "$DISCOVER_BIN" ] && which -global-discover >/dev/null 2>&1 && DISCOVER_BIN=$(which -global-discover)
[ -z "$DISCOVER_BIN" ] && [ -f bin/-global-discover.ts ] && DISCOVER_BIN="bun run bin/-global-discover.ts"
...
- Top Work: 3 bullet points summarizing the user's major themes, inferred from
  ...
  not "feat: -global-discover" + "feat: /retro global template".
...
This is the "deep dive" thatshareable card.
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
today=$(date +%Y-%m-%d)
existing=$(ls ${today}-*.json 2>/dev/null | wc -l | tr -d ' ')
next=$((existing + 1))
```

Use the Write tool to save JSON to `${today}-${next}.json`:
...
```json
{
  ...
  "tweetable": "Week of Mar 14: 5 projects, 182 commits, 15.3k LOC | CC: 48, Codex: 8, Gemini: 3 | Focus:  (58%) | Streak: 52d"
}
```
...
- **Global mode:** Does NOT require being inside a git repo. Saves snapshots to `` (not `.context/retros/`). Gracefully skip AI tools that aren't installed. Only compare against prior global retros with the same window value. If streak hits 365d cap, display as "365+ days".
