...
description: "Land and deploy workflow. Merges PR, waits for CI and deploy, then verifies production health via canary checks."
category: deployment
tags: [deploy, merge, ci-cd]
---
...
```bash
eval "$( 2>/dev/null)"
if [ ! -f $SLUG/land-deploy-confirmed ]; then
  echo "FIRST_RUN"
else
  # Check if deploy config has changed since confirmation
  SAVED_HASH=$(cat $SLUG/land-deploy-confirmed 2>/dev/null)
...
CURRENT_HASH=$(sed -n '/## Deploy Configuration/,/^## /p' CLAUDE.md 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
WORKFLOW_HASH=$(find .github/workflows -maxdepth 1 \( -name '*deploy*' -o -name '*cd*' \) 2>/dev/null | xargs cat 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
echo "${CURRENT_HASH}-${WORKFLOW_HASH}" > $SLUG/land-deploy-confirmed
```
Continue to Step 2.
...
ls -t ~/.-dev/evals/*-e2e-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -20
```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t ~/.-dev/evals/*-llm-judge-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -5
```
...
Then run `-diff-scope` to classify the changes:
...
Save report to `./deploy-reports/{date}-pr{number}-deploy.md`.
