...
description: "Auto-review pipeline that runs CEO, design, and eng reviews sequentially with auto-decisions. Surfaces only taste decisions at final approval."
category: planning
tags: [review, pipeline, automation]
---
...
DESIGN=$(ls -t $SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t $SLUG/*-design-*.md 2>/dev/null | head -1)
...
echo "RESTORE_PATH=$HOME/./projects/$SLUG/${BRANCH}-autoplan-restore-${DATETIME}.md"
```
...
- Discover design docs: `ls -t $SLUG/*-design-*.md 2>/dev/null | head -1`
...
- Test plan: generate artifact at `$SLUG/{user}-{branch}-test-plan-{datetime}.md`
...
- [ ] Test plan artifact written to disk at $SLUG/
- [ ] "NOT in scope" section written
- [ ] "What already exists" section written
